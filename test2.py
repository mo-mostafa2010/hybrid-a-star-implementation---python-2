...
class States(object):
    ...
    def __init__(self, sx, sy, syaw, mx, my, myaw, gx, gy, gyaw):
        self.sx = sx
        self.sy = sy
        self.syaw = syaw
        self.mx = mx
        ...
        self.gy = gy
        self.gyaw = gyaw
        self.estimated_cost = self.estimate_total_rs_cost(sx, sy, syaw, mx, my, myaw, gx, gy, gyaw)

    def estimate_rs_cost(self, sx, sy, syaw, gx, gy, gyaw):

        max_curvature = math.tan(planner.MAX_STEER) / planner.WB
        paths = planner.rs_path.calc_paths(sx, sy, syaw, gx, gy, gyaw,
                                           max_curvature, step_size=planner.MOTION_RESOLUTION)
        if len(paths) == 0:
            return None

        rs_path_cost = planner.calc_rs_path_cost(paths[0])
        for path in paths:
            cost = planner.calc_rs_path_cost(path)
            if cost < rs_path_cost:
                rs_path_cost = cost
        return rs_path_cost

    def estimate_total_rs_cost(self, sx, sy, syaw, mx, my, myaw, gx, gy, gyaw):
        cost1 = self.estimate_rs_cost(sx, sy, syaw, mx, my, myaw)
        cost2 = self.estimate_rs_cost(mx, my, myaw, gx, gy, gyaw)
        if cost1 == None or cost2 == None:
...
def main(x,y,yaw):
    ...
    # path generation
    path1 = planner.calc_hybrid_astar_path(sx, sy, syaw0, mx, my, myaw0, ox, oy,
    ...
    path3 = planner.calc_hybrid_astar_path(mx, my, myaw0, gx, gy, gyaw0, ox, oy,
                                          planner.XY_GRID_RESOLUTION,
                                          planner.YAW_GRID_RESOLUTION)
...
#     states = States(SX, SY, SYAW, mx, my, myaw, GX, GY, GYAW)
...
#     del states_set[s_id]
#     print states.mx, states.my, states.myaw
